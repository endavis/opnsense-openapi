...
`src/opnsense_openapi/cli.py` and is registered as a console script via
...
requires the application CLI to live under `src/opnsense_openapi/` and to be
...
- The template now has a concrete, testable CLI surface that contributors
  can extend by copying the pattern in `src/opnsense_openapi/cli.py`.
- The runtime/dev boundary is enforceable: end users of the published
  package run `opnsense-openapi ...` without needing `doit` installed.
