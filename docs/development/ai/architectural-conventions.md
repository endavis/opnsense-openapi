...
- DO put runtime application code under `src/opnsense_openapi/`.
- DO put development tooling, scaffolding, install scripts, and doit task definitions under `tools/`.
- DO expose the application's user-facing command-line interface as a console script declared in `[project.scripts]` in `pyproject.toml`, backed by a module under `src/opnsense_openapi/`.
...
- DO NOT import from `tools/` or `dodo.py` in any module under `src/opnsense_openapi/`. Runtime code must be installable and runnable without any dev tooling present.
...
**Right:** Add a module under `src/opnsense_openapi/` that implements `greet`, register a console script in `[project.scripts]` (e.g. `greet = "opnsense_openapi.greet:main"`), and test it as runtime code under `tests/`. End users install the package and run `greet` directly — they should never need `doit` installed.

### 2. Importing a helper from `tools/` into `src/opnsense_openapi/`
...
**Right:** If the helper is needed by runtime code, move it into `src/opnsense_openapi/` and test it as runtime code. If it is only needed by dev tooling, duplicate the small bit of logic or keep it in `tools/` — `src/opnsense_openapi/` never imports from `tools/` or `dodo.py`.
...
**Wrong:** Agent drafts a README section telling end users to `pip install opnsense_openapi && doit run_app` to launch the application.
...
2. Identify the layer the change belongs in: runtime (`src/opnsense_openapi/`), dev tooling (`tools/`), or dev entry point (`dodo.py`).
3. Confirm imports respect the boundary: nothing under `src/opnsense_openapi/` imports from `tools/` or `dodo.py`.
