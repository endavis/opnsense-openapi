...
def task_profile():
    ...
    return {
        "actions": [
            "uv run py-spy record -o tmp/profile.svg -- python -m opnsense_openapi",
        ],
...
def task_profile_memory():
    ...
    return {
        "actions": [
            "memray run -o tmp/memray.bin python -m opnsense_openapi",
...
```python
# src/opnsense_openapi/plugin_interface.py
...
```python
# src/opnsense_openapi/plugin_loader.py
...
def discover_plugins() -> Dict[str, PluginInterface]:
    ...
    # Find all entry points in 'opnsense_openapi.plugins' group
    eps = entry_points(group='opnsense_openapi.plugins')
...
```toml
[project.entry-points."opnsense_openapi.plugins"]
default = "opnsense_openapi.plugins.default:DefaultPlugin"
csv_export = "opnsense_openapi.plugins.csv_export:CSVExportPlugin"
```
...
pip install opnsense-openapi-analytics
```
...
```toml
# Your core package
[project.entry-points."opnsense_openapi.plugin_types"]
processor = "opnsense_openapi.plugin_types.processor:ProcessorPluginType"
exporter = "opnsense_openapi.plugin_types.exporter:ExporterPluginType"

# Register built-in plugins by type
[project.entry-points."opnsense_openapi.processors"]
csv = "opnsense_openapi.plugins.csv_processor:CSVProcessor"
json = "opnsense_openapi.plugins.json_processor:JSONProcessor"

[project.entry-points."opnsense_openapi.exporters"]
s3 = "opnsense_openapi.plugins.s3_exporter:S3Exporter"
local = "opnsense_openapi.plugins.local_exporter:LocalExporter"
```

**Plugin Type Manager:**

```python
# src/opnsense_openapi/plugin_types/processor.py
...
class ProcessorPluginType:
    ...
    def entry_point_group(self) -> str:
        return "opnsense_openapi.processors"
...
for ep in entry_points(group='opnsense_openapi.cli_plugins'):
    try:
        command = ep.load()
...
```toml
[project.entry-points."opnsense_openapi.cli_plugins"]
...
from opnsense_openapi.plugin_loader import discover_plugins
...
# Run the application
CMD ["uv", "run", "python", "-m", "opnsense_openapi"]
```
