...
| :--- | :--- | :--- | :--- |
| **Runtime** | `src/opnsense_openapi/` | End users of the published package | The actual application/library code that gets shipped to PyPI. |
...
> **`src/opnsense_openapi/` never imports from `tools/` or `dodo.py`.**
...
| :--- | :--- | :--- | :--- |
| **Application console script** | The package's user-facing command-line interface, defined as a `[project.scripts]` entry under `src/opnsense_openapi/`. | End users of the published package. | Development workflow tasks. See the [CLI Guide](../usage/cli.md) for the application CLI. |
...
- **The application's user-facing CLI is a console script under
  `src/opnsense_openapi/`, not a doit task.** End users should never need to
