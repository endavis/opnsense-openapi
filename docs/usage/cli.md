...
interface** — the `opnsense-openapi` console script shipped by the published
...
After installing the package, the `opnsense-openapi` command is available on
your `PATH`:

```bash
$ opnsense-openapi greet
Hello, World!

$ opnsense-openapi greet --name Python
Hello, Python!

$ opnsense-openapi greet -n Python
Hello, Python!

$ opnsense-openapi --version
opnsense_openapi, version 0.0.0
```
...
```bash
uv run opnsense-openapi greet --name Python
```
...
```toml
[project.scripts]
opnsense-openapi = "opnsense_openapi.cli:main"
```

The script name on the left (`opnsense-openapi`) is the hyphen-cased
distribution name. The value on the right points at the `main` function
in `src/opnsense_openapi/cli.py`. When the package is installed (via `uv
sync`, `pip install`, etc.), the build backend generates an executable
shim that calls `opnsense_openapi.cli.main()`.

The CLI module lives at `src/opnsense_openapi/cli.py` and is a single file
...
from opnsense_openapi.core import greet as _greet


@click.group()
@click.version_option(opnsense_openapi="opnsense_openapi")
def main() -> None:
    """opnsense_openapi command-line interface."""
...
- **`@click.version_option(opnsense_openapi="opnsense_openapi")`** wires up
...
- **Subcommands call into the runtime package.** The `greet` command
  delegates to `opnsense_openapi.core.greet`. The CLI layer handles parsing
...
1. **Open `src/opnsense_openapi/cli.py`.**
...
3. **Delegate to a function in the runtime package.** Do not put business
   logic in `cli.py` — call into `opnsense_openapi.core` (or a new module)
...
```bash
$ opnsense-openapi shout python --exclaim 3
Hello, PYTHON!!!
```
