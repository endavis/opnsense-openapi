...
> Looking for the `opnsense-openapi` command-line interface? See the
...
def task_run_server():
    ...
    return {
        "actions": ["uv run opnsense-openapi server --debug --port 8080"],
...
def task_init_db():
    ...
    return {
        "actions": [
            "uv run opnsense-openapi db create",
            "uv run opnsense-openapi db migrate",
            "uv run opnsense-openapi db seed-dev",
        ],
        "title": title_with_actions,
    }
```
...
def task_deploy_dev():
    ...
    return {
        "actions": [
            "uv run opnsense-openapi validate --env dev",
            "uv run opnsense-openapi deploy --env dev --auto-approve",
        ],
        "title": title_with_actions,
    }
...
def task_deploy_prod():
    ...
    def check_and_deploy():
        ...
        # Run deployment
        subprocess.run(
            "uv run opnsense-openapi deploy --env prod",
            shell=True,
            check=True
        )
...
def task_process_data():
    ...
    return {
        "actions": [
            "uv run opnsense-openapi extract --source api --output tmp/raw.json",
            "uv run opnsense-openapi transform --input tmp/raw.json --output tmp/clean.json",
            "uv run opnsense-openapi load --input tmp/clean.json --target postgres",
        ],
        "title": title_with_actions,
    }
...
def task_backup_data():
    ...
    return {
        "actions": [
            "uv run opnsense-openapi backup create --env prod --output backups/$(date +%Y%m%d-%H%M%S).sql.gz",
        ],
        "title": title_with_actions,
    }
...
def task_restore_data():
    ...
    def restore_latest():
        ...
        subprocess.run(
            f"uv run opnsense-openapi backup restore --file {latest} --env dev",
            shell=True,
            check=True
        )
...
def task_deploy():
    ...
    def deploy_to_env(env):
        ...
        # Deploy
        subprocess.run(
            f"uv run opnsense-openapi deploy --env {env}",
            shell=True,
            check=True
        )
...
def task_validate_config():
    ...
    return {
        "actions": [
            "uv run opnsense-openapi config validate --env dev",
            "uv run opnsense-openapi config validate --env prod",
        ],
        "title": title_with_actions,
    }
...
def task_health_check():
    ...
    return {
        "actions": [
            ...
            "uv run opnsense-openapi db ping",
            "uv run opnsense-openapi cache status",
        ],
        "title": title_with_actions,
    }
```
...
def task_generate_models():
    ...
    return {
        "actions": [
            "uv run datamodel-codegen --input api-spec.yaml --output src/opnsense_openapi/models/",
        ],
        "file_dep": ["api-spec.yaml"],
        "targets": ["src/opnsense_openapi/models/api.py"],
        "title": title_with_actions,
    }
...
# Check specific file
uv run mypy src/opnsense_openapi/core.py
```
