...
# Start development
uv run python -m opnsense_openapi
```
...
# Run with arguments
uv run python -m opnsense_openapi --verbose
```
...
3. Set **Module name**: `opnsense_openapi`
...
```bash
# Using pdb
uv run python -m pdb -m opnsense_openapi
...
# Verify installation
uv run python -c "import opnsense_openapi; print(opnsense_openapi.__version__)"
```
...
uv run uvicorn opnsense_openapi.app:app --reload

# Using Flask debug mode
FLASK_DEBUG=1 uv run flask run

# Using watchfiles for custom scripts
uv run watchfiles "python -m opnsense_openapi" src/
```
...
```bash
# Using cProfile
uv run python -m cProfile -o profile.stats -m opnsense_openapi
...
uv pip install --index-url https://test.pypi.org/simple/ opnsense-openapi

# If your package has dependencies from real PyPI, use both indexes
uv pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ opnsense-openapi
```
...
# https://test.pypi.org/project/opnsense-openapi/

# Verify installation works
uv venv /tmp/test-install
uv pip install --python /tmp/test-install --index-url https://test.pypi.org/simple/ opnsense-openapi
```
