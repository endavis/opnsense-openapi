# opnsense-openapi – AI Agent Instructions

## Overview
**Reference:** [README.md](README.md)

Lightweight Python wrapper generator for OPNsense API endpoints. The tool takes
an OPNsense version, downloads the relevant API files for that version, and
generates a Python wrapper for that version's API.

Built on the `pyproject-template` structure: `uv` for environment management,
`doit` as task runner, `ruff` + `mypy` for lint/type checks, `hatch-vcs` for
git-tag-driven versioning.
