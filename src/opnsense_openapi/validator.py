...
class SpecValidator:
    ...
    def __init__(self, client: OPNsenseClient, spec_path: Path) -> None:
        ...
        self.resolver = jsonschema.RefResolver(
            base_uri=f"file://{spec_path.absolute()}", referrer=self.spec
        )
    ...
    def validate_endpoints(self, max_endpoints: int = 50) -> Generator[dict[str, Any], None, None]:
        ...
        for path, methods in paths.items():
            ...
            count += 1
            result = {"path": path, "method": "GET", "valid": False, "error": None, "status": 0}
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    response_content_schema_block = methods["get"]["responses"]["200"].get(
                        "content"
                    )

                    # Determine if the spec expects JSON content for 200 OK
                    spec_expects_json = (
                        response_content_schema_block
                        and "application/json" in response_content_schema_block
                    )
                    ...
                    if spec_expects_json:
                        ...
                        if "application/json" in content_type:
                            try:
                                data = response.json()
                                response_schema = response_content_schema_block["application/json"][
                                    "schema"
                                ]
                                ...
                                result["error"] = (
                                    "Invalid JSON response (server returned malformed JSON "
                            ...
                            result["error"] = (
                                f"Content-Type mismatch: Server returned '{content_type}' "
                                "but schema expected 'application/json'."
                            )
                    else:
                        # Spec does NOT explicitly expect JSON for 200 OK
                        # (e.g., it might expect text/csv, or no content)
                        # Assume valid if HTTP 200 OK, skip schema validation.
                        result["valid"] = True
                        result["error"] = (
                            f"Schema does not explicitly expect JSON for 200 OK. "
