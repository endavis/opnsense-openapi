...
Tests invariant properties of utility functions in
``tools.pyproject_template.utils`` using randomly generated inputs rather than
hand-picked examples.

The upstream template's ``test_properties.py`` also tests
``package_name.core.greet``; that scaffolding module is not adopted in this
project (see issue #12 plan, Scope OUT) so the related ``TestGreetProperties``
